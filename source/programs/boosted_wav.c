...
void cut_wave_file(const char* in, const char* out, int fastdownFactor){
	...
	WavHeader out_header = create_extendedHeader(&in_header, fastdownFactor);
	if (PRINT_HEADER){
		printf("\nOut header:\n");
	...
	uint32_t numSamples = fastdownFactor * out_header.subchunk2Size / blockSize;
